...
{
    public partial class GACC_CoordinadorListarCostosIndirecto1 : System.Web.UI.Page
    {
        ...
        {
            List<GACC_ViewCosto> listaproductos = new List<GACC_ViewCosto>();
            listaproductos = GACC_ControladorCostoIndirecto.ObtenerCostoIndirecto();
        ...
        protected void gacc_lnknuevo_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("GACC_CoordinadorInsertarCostosIndirecto.aspx");
        }

        protected void gacc_lnkbuscar_Click(object sender, EventArgs e)
        {
            List<GACC_TblCostoIndirecto> list = new List<GACC_TblCostoIndirecto>();
            GACC_TblCostoIndirecto pro = new GACC_TblCostoIndirecto();
            ...
            if (op != "0")
            {
                ...
                {
                    case "N":
                        pro = GACC_ControladorCostoIndirecto.ObtenerCostoIxnombre(gacc_txtbuscar.Text);
                        if (pro != null)
                        {
                            list.Add(pro);
                        ...
                        {
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('No se encontro el Costo')", true);
                            cargarNombreProyeccto();
                        }
                        break;
                }
        ...
        protected void gacc_grv_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                Response.Redirect("GACC_CoordinadorInsertarCostosIndirecto.aspx?cod=" + codigo, true);
            }
            else if (e.CommandName.Equals("Eliminar"))
            {
                GACC_TblCostoIndirecto prod = new GACC_TblCostoIndirecto();
                prod = GACC_ControladorCostoIndirecto.ObtenerCostoIxid(codigo);
                if (prod != null)
                {
                    GACC_ControladorCostoIndirecto.delete(prod);
                    cargarNombreProyeccto();
                }
